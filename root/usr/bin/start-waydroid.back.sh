...
if [ -z "$WAYLAND_DISPLAY" ]; then
    export WAYLAND_DISPLAY=wayland-1
        ...
        # Simple start:
        weston --backend=x11-backend.so --fullscreen --log=/tmp/weston.log > /tmp/weston.out 2>&1 &
        WESTON_PID=$!
        ...
        while [ ! -e "$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY" ]; do
            if ! kill -0 $WESTON_PID 2>/dev/null; then
                echo "Weston died prematurely! Check /tmp/weston.log and /tmp/weston.out"
                cat /tmp/weston.out
                exit 1
            fi
            sleep 1
...
# Using show-full-ui to launch the app window in Weston
